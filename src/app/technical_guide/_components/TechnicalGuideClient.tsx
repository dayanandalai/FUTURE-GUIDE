            ...
            {subjects.map((subject) => {
              ...
              return (
                <Link
                  href={`/${slug}`}
                  key={subject.id}
                  className={`w-full text-left text-black text-lg p-4 rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 focus:outline-none ${selectedSubject?.id === subject.id ? 'bg-white shadow-lg scale-105' : 'bg-gray-300 bg-opacity-70 hover:bg-gray-200'}`}
